...
class MontgomeryCurve {
  ...
  doubleAffinePoint(p) {
    let tmpX = math.mod(
      BigInt("3") * p.x * p.x + BigInt("2") * p.x * this.coefA + BigInt("1"),
      this.prime
    );
    ...
    let newY = math.mod(
      (p.x * BigInt("3") + this.coefA - b_l2) * l1 - p.y,
      this.prime
    );
  ...
  addAffineG1(p1, p2) {
    if (p1.inf && p2.inf) {
      return new AffinePoint(0, 0, true);
    }
    if (p1.inf) {
      return p2;
    }
  ...
  doubleAffineG1(p) {
    let s = math.mod(
      (3 * p.x * p.x + 2 * this.coefA * p.x + 1) *
        this.fieldDivision(1, 2 * this.coefB * p.y),
      this.prime
    );
  }
  ...
  checkPointOnCurve(p) {
    let lhs = math.mod(
      math.modPow(p.y, BigInt("2"), this.prime) * this.coefB,
      this.prime
    );
    let rhs = math.mod(
      math.modPow(p.x, BigInt("3"), this.prime) +
        this.coefA * math.modPow(p.x, BigInt("2"), this.prime) +
        p.x,
      this.prime
    );
...
export class WeierstrassCurve {
  ...
  subAffineG1(p1, p2) {
    const negP2 = new AffinePoint(p2.x, math.mod(-p2.y, this.PRIME));

    return this.addAffineG1(p1, negP2);
  }
  ...
  scalarMulG1(p, n) {
    if (typeof n !== "bigint") {
      throw new Error("n must be bigint type");
    }
    let tmp = p;
