...
class MontgomeryCurve {
    ...
    /**
     *
     * @param {BigInt, BigInt} p The point (X, Y)
     * @returns {AffinePoint}   Change to affine point
     */
    ...
    computeYCoord(x) {
        let squared = math.mod(
            x * x * x + this.coefA * (x * x) + x,
            this.prime
        );
    ...
    multscalar(baseX, exp) {
        let baseY = this.computeYCoord(baseX);
        let bp = new AffinePoint(baseX, baseY);
        this.checkPointOnCurve(bp);
        let result = this.computeScalarMul(bp, exp);
        this.checkPointOnCurve(result);
