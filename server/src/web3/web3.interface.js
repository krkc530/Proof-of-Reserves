...
export default class Web3Interface extends Web3 {
   ...
   */
  constructor(endpoint, senderEthAddr, senderEthPrivateKey) {
    super(endpoint);
    // nonce managing for fast async transaction
    this.senderEthAddr = senderEthAddr;
    this.senderEthPrivateKey = senderEthPrivateKey;
    this.eth
      .getTransactionCount(senderEthAddr, "pending")
      .then((nonce) => (this.nonce = nonce));
  }

  // for async transaction
  ...
  async hasMethod(contractAddress, signature) {
    ...
    return (
      code.indexOf(functionSignature.slice(2, functionSignature.length)) > 0
    );
  }
   ...
   */
  async sendContractCall(call, gas = "150000", value) {
    /*
    ...
    if (this.senderEthPrivateKey !== undefined) {
      ...
      const rawTx = {
        ...
        nonce: addPrefixHex(this.nonce.toString(16)),
        common: {
          customChain: {
            networkId: chainId,
            chainId: chainId,
  ...
  async sendTransaction(
    ...
    try {
      ...
      if (this.senderEthPrivateKey !== undefined) {
        const txCount = await this.eth.getTransactionCount(
          this.senderEthAddr,
          "pending"
        );
        const chainId = await this.eth.getChainId();
